...
import static battlecode.common.GameActionExceptionType.CANT_DO_THAT;
...
public class RobotControllerImpl extends ControllerShared implements RobotController, GenericController {
    ...
    public void spawn(Direction dir) throws GameActionException {
        ...
        if (robot.type != RobotType.HQ)
            throw new GameActionException(CANT_DO_THAT, "Only HQs can spawn.");
        assertNotMoving();
    ...
    public void captureEncampment(RobotType type) throws GameActionException {
    	if (robot.type != RobotType.SOLDIER)
            throw new GameActionException(CANT_DO_THAT, "Only SOLDIERs can capture encampments.");
    	if (!type.isEncampment)
            throw new GameActionException(CANT_DO_THAT, "Must specify a valid encampment type to create");
    	assertNotMoving();
    ...
    public void researchUpgrade(Upgrade upgrade) throws GameActionException {
    	if (robot.type != RobotType.HQ)
            throw new GameActionException(CANT_DO_THAT, "Only HQs can research.");
    	if (gameWorld.hasUpgrade(getTeam(), upgrade))
    		throw new GameActionException(CANT_DO_THAT, "You already have that upgrade. ("+upgrade+")");
    	assertNotMoving();
    ...
    public void layMine() throws GameActionException {
    	if (robot.type != RobotType.SOLDIER)
            throw new GameActionException(CANT_DO_THAT, "Only SOLDIERs can lay mines.");
    	assertNotMoving();
    ...
    private void stopMine() throws GameActionException {
    	...
    	if (robot.getMiningRounds() == 0)
    		throw new GameActionException(CANT_DO_THAT, "You are not mining currently");
...
//    	if (robot.type != RobotType.SOLDIER)
//            throw new GameActionException(CANT_DO_THAT, "Only SOLDIERs can scan for mines.");
...
////                && gameWorld.inAngleRange(myLoc, getDirection(), loc, robot.type.sensorCosHalfTheta);
    ...
    public boolean senseEnemyNukeHalfDone() throws GameActionException {
    	if (getRobot().type != RobotType.HQ)
    		throw new GameActionException(CANT_DO_THAT, "Only HQs can sense NUKE progress");
    ...
    public MapLocation[] senseEncampmentSquares(final MapLocation center, final int radiusSquared, final Team team) throws GameActionException {
    	if (team == getTeam().opponent())
    		throw new GameActionException(CANT_DO_THAT, "can't sense enemy encampments");
...
//    }
    ...
    public void move(Direction d) throws GameActionException {
        if (robot.type != RobotType.SOLDIER)
        	throw new GameActionException(CANT_DO_THAT, "Only SOLDIERs can move.");
    	assertNotMoving();
        assertCanMove(d);
        int delay = 1;
...
//        if (robot.type != RobotType.SOLDIER)
//        	throw new GameActionException(CANT_DO_THAT, "Only SOLDIER make melee attacks.");
//    	assertNotAttacking();
//    	
...
//    }
    ...
    public void attackSquare(MapLocation loc) throws GameActionException {
        if (robot.type != RobotType.ARTILLERY)
        	throw new GameActionException(CANT_DO_THAT, "Only ARTILLERY can attack a targeted square.");
        assertNotAttacking();
        assertNotNull(loc);
    ...
    public void broadcast(int channel, int data) throws GameActionException {
    	if (channel<0 || channel>GameConstants.BROADCAST_MAX_CHANNELS)
    		throw new GameActionException(CANT_DO_THAT, "Can only use radio channels from 0 to "+GameConstants.BROADCAST_MAX_CHANNELS+", inclusive");
    ...
    public void wearHat() throws GameActionException {
    	if (getType() != RobotType.SOLDIER)
    		throw new GameActionException(CANT_DO_THAT, "Only SOLDIERS can wear hats.");
